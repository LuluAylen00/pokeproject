...
function getRecovery(gen, attacker, defender, move, damage, notation) {
    ...
    text = "".concat(minHealthRecovered, " - ").concat(maxHealthRecovered).concat(notation, " recuperado");
    return { recovery: recovery, text: text };
}
...
function getRecoil(gen, attacker, defender, move, damage, notation) {
    ...
    if (move.recoil) {
        ...
        if (!attacker.hasAbility('Rock Head')) {
            ...
            text = "".concat(minRecoilDamage, " - ").concat(maxRecoilDamage).concat(notation, " da\u00F1o de retroceso");
        }
    }
...
function getKOChance(gen, attacker, defender, move, field, damage, err) {
    ...
    if ((move.timesUsed === 1 && move.timesUsedWithMetronome === 1) || move.isZ) {
        ...
        if (chance === 1) {
            return { chance: chance, n: 1, text: "KO en 1 golpe garantizado".concat(hazardsText) };
        }
        else if (chance > 0) {
            return {
                chance: chance,
                n: 1,
                text: qualifier + Math.round(chance * 1000) / 10 + "% posibilidades de KO en 1 golpe".concat(hazardsText)
            };
        }
        ...
        for (var i = 2; i <= 4; i++) {
            ...
            if (chance_1 === 1) {
                return { chance: chance_1, n: i, text: "".concat(qualifier || 'garantizado KO', " en ").concat(i, " golpes").concat(afterText) };
            }
            else if (chance_1 > 0) {
                return {
                    chance: chance_1,
                    n: i,
                    text: qualifier + Math.round(chance_1 * 1000) / 10 + "% chances de KO en ".concat(i, " golpes ").concat(afterText)
                };
            }
        }
        for (var i = 5; i <= 9; i++) {
            if (predictTotal(damage[0], eot.damage, i, 1, toxicCounter, defender.maxHP()) >=
                ...
                return { chance: 1, n: i, text: "".concat(qualifier || 'KO en ').concat(i, " garantizado").concat(afterText) };
            }
            else if (predictTotal(damage[damage.length - 1], eot.damage, i, 1, toxicCounter, defender.maxHP()) >=
                defender.curHP() - hazards.damage) {
                return { n: i, text: qualifier + "posible KO en ".concat(i, " golpes").concat(afterText) };
            }
        }
    }
    else {
        ...
        if (chance === 1) {
            return {
                chance: chance,
                n: move.timesUsed,
                text: "".concat(qualifier || 'garantizado ', "KO en ").concat(move.timesUsed, " turnos").concat(afterText)
            };
        }
        else if (chance > 0) {
            return {
                ...
                text: qualifier +
                    ...
                    "% chances de KO en ".concat(move.timesUsed, " golpes").concat(afterText)
            };
        }
        if (predictTotal(damage[0], eot.damage, move.hits, move.timesUsed, toxicCounter, defender.maxHP()) >=
            ...
            return {
                n: move.timesUsed,
                text: qualifier + "posible KO en ".concat(move.timesUsed, " turnos").concat(afterText)
            };
        }
...
function buildDescription(description, attacker, defender) {
    ...
    if (description.alliesFainted) {
        output += Math.min(5, description.alliesFainted) +
            " ".concat(description.alliesFainted === 1 ? 'aliado' : 'aliados', " debilitados ");
    }
...
function getDescriptionLevels(attacker, defender) {
    if (attacker.level !== defender.level) {
        return [
            attacker.level === 100 ? '' : "Nv ".concat(attacker.level),
            defender.level === 100 ? '' : "Nv ".concat(defender.level),
        ];
    }
    var elide = [100, 50, 5].includes(attacker.level);
    var level = elide ? '' : "Nv ".concat(attacker.level);
    return [level, level];
}
