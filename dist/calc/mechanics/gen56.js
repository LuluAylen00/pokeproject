...
function calculateBWXY(gen, attacker, defender, move, field) {
    ...
    var noTypeChange = move.named('Sentencia', 'Adaptación', 'Tecno Shock', 'Don Natural', 'Meteorobola');
    ...
    if (attacker.hasAbility('Gale Wings') && move.hasType('Volador')) {
        move.priority = 1;
        desc.attackerAbility = attacker.ability;
    }
    ...
    else if (typeEffectiveness === 0 && move.hasType('Tierra') &&
    ...
    if ((attacker.hasAbility('Technician') && basePower <= 60) ||
        ...
        move.hasType('Roca', 'Tierra', 'Acero')) {
        bpMods.push(5325);
        desc.attackerAbility = attacker.ability;
        desc.weather = field.weather;
    }
    ...
    var attack;
    var attackSource = move.named('Juego Sucio') ? defender : attacker;
    ...
    if ((field.hasWeather('Sun', 'Harsh Sunshine') && move.hasType('Fuego')) ||
