...
function performCalculations() {
	...
	bestResult.change();
	$("#resultHeaderL").text(`Movimientos de ${p1.name} (Selecciona uno para ver los resultados detallados)`);
	$("#resultHeaderR").text(`Movimientos de ${p2.name} (Selecciona uno para ver los resultados detallados)`);
}
...
$(".result-move").change(function () {
	if (damageResults) {
		...
		if (result) {
			...
			if (desc.indexOf('--') === -1) desc += ' -- posiblemente el peor movimiento posible';
			$("#mainResult").text(desc);
			$("#damageValues").text("Cantidades posibles de daño: (" + displayDamageHits(result.damage) + ")");
		}
	}
});
...
function displayDamageHits(damage) {
	...
	if (typeof damage[0] === 'number' && typeof damage[1] === 'number') {
		return '1er Golpe: ' + damage[0] + '; 2do Golpe: ' + damage[1];
	}
	// Parental Bond Damage
	return '1er Golpe: ' + damage[0].join(', ') + '; 2do Golpe: ' + damage[1].join(', ');
}
